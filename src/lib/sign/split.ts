...
function capProfiles(
  ...
  for (const profile of profiles) {
    ...
    vertices.push(...a.to…1671 tokens truncated…   connectorTriangles.flatMap((triangle) => triangle.flatMap((point) => point.toArray())),
        3,
      ),
    );
    return cap;
...
export function splitGeometryByPlane(
  ...
  // A parte inferior usa a metade interna da parede; a superior recebe o
  // rebaixo invertido na metade externa. Gerar a fÃªmea a partir do perfil
