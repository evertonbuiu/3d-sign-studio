...
function idsOrSelection(state: SketchState, ids?: string[]): Set<string> {
  return new Set(ids ?? state.selectedIds);
}

/** Centro da caixa envolvente das entidades selecionadas. */
export function selectionCenter(state: SketchState, ids?: string[]): SketchPoint | null {
  const selected = idsOrSelection(state, ids);
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
...
/** Move entidades no plano XY mantendo extrusões e seleção vinculadas. */
export function translateEntities(
  ...
  const selected = idsOrSelection(state, ids);
  if (!selected.size || !Number.isFinite(dx) || !Number.isFinite(dy) || (!dx && !dy)) return state;
  return {
    ...state,
    entities: state.entities.map((entity) => {
      ...
      if (entity.type === "rect") return { ...entity, x: entity.x + dx, y: entity.y + dy };
      return { ...entity, cx: entity.cx + dx, cy: entity.cy + dy };
    }),
  };
}

function rotatePoint(point: SketchPoint, origin: SketchPoint, radians: number): SketchPoint {
...
/** Rotaciona entidades ao redor do centro conjunto da seleção. */
export function rotateEntities(
  ...
  const selected = idsOrSelection(state, ids);
  const origin = selectionCenter(state, [...selected]);
  if (!selected.size || !origin || !Number.isFinite(degrees) || !degrees) return state;
  const radians = (degrees * Math.PI) / 180;
  return {
    ...state,
    entities: state.entities.map((entity): SketchEntity => {
      if (!selected.has(entity.id)) return entity;
      if (entity.type === "circle") {
        const center = rotatePoint({ x: entity.cx, y: entity.cy }, origin, radians);
        return { ...entity, cx: center.x, cy: center.y };
      }
      const points = entityPoints(entity).map((point) => rotatePoint(point, origin, radians));
      return {
        id: entity.id,
        type: "polyline",
        points,
        closed: entity.type !== "polyline" || entity.closed,
      };
    }),
  };
}

/** Escala uniformemente entidades ao redor do centro conjunto da seleção. */
export function scaleEntities(
  ...
  const selected = idsOrSelection(state, ids);
  const origin = selectionCenter(state, [...selected]);
  if (!selected.size || !origin || !Number.isFinite(factor) || factor <= 0 || factor === 1)
    return state;
  const scalePoint = (point: SketchPoint): SketchPoint => ({
  ...
  return {
    ...
    entities: state.entities.map((entity): SketchEntity => {
      if (!selected.has(entity.id)) return entity;
      if (entity.type === "polyline") {
        return { ...entity, points: entity.points.map(scalePoint) };
      }
      if (entity.type === "rect") {
        const corner = scalePoint({ x: entity.x, y: entity.y });
        return {
          ...entity,
          x: corner.x,
          y: corner.y,
      ...
      const center = scalePoint({ x: entity.cx, y: entity.cy });
      return { ...entity, cx: center.x, cy: center.y, radius: entity.radius * factor };
    }),
  };
}

/** Duplica a seleção, incluindo suas extrusões, e desloca a cópia para ficar visível. */
export function duplicateEntities(
  state: SketchState,
  ids: string[] | undefined = undefined,
  offset: SketchPoint = { x: 10, y: 10 },
): SketchState {
  const selected = idsOrSelection(state, ids);
  ...
  for (const entity of state.entities) {
    if (!selected.has(entity.id)) continue;
    const id = nextSketchId("e");
    idMap.set(entity.id, id);
    if (entity.type === "polyline") {
      copies.push({ ...entity, id, points: entity.points.map((point) => ({ ...point })) });
    } else {
      copies.push({ ...entity, id });
    }
  }
  const copiedIds = copies.map((entity) => entity.id);
  const extrusions = state.extrusions.flatMap((extrusion) => {
    const entityId = idMap.get(extrusion.entityId);
    return entityId ? [{ ...extrusion, id: nextSketchId("x"), entityId }] : [];
  });
  return translateEntities(
    {
      ...state,
      entities: [...state.entities, ...copies],
      extrusions: [...state.extrusions, ...extrusions],
